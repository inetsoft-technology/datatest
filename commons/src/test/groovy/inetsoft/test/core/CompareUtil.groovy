...
class CompareUtil {
   ...
   def compareData(String asset_id, String[] fileNames, String packageName) {
      ...
      if(fileNames != null) {
         fileNames.each {
            resPath = exportUtil.getExportFolderPath(asset_id, packageName) + File.separator + it + '.txt'
            resArray.add(FileCompare(resPath))
         }
      }
      else {
         File folder = new File(exportUtil.getExportFolderPath(asset_id, packageName))
         folder.listFiles().each {
            resPath = it.getAbsolutePath()
            if(it.getAbsolutePath().endsWith('.txt') || it.getAbsolutePath().endsWith('.html')) {
               resArray.add(FileCompare(resPath))
            }
         }
      }
   ...
   void compareImage(String asset_id, String[] fls, String packageName) {
      ...
      if(fls != null) {
         fls.each {
            resPath = exportUtil.getExportFolderPath(asset_id, packageName) + File.separator + it + '.png'
            if( new File(resPath).exists() && !resPath.endsWith('_diff.png')){
